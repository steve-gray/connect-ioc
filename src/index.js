...
const debug = require('debug')('connect-ioc');
...
function autoRegister(container, settings) {
  ...
  glob(settings.pattern, {
    ...
    for (const moduleName of files) {
      ...
      const loaded = require(qualifiedPath); // eslint-disable-line global-require
...
/**
 * Middleware for performing IoC integration with a Connect/Express request.
 * Uses somersault to create a root container, if no root is specified.
 *
...
module.exports = (iocOptions) => {
  ...
  const middleware = (req, res, next) => {
    ...
    // Attach to request
    req.ioc = requestContainer; // eslint-disable-line no-param-reassign
